...
| C3 | Every artifact's lineage is reconstructable and replayable | git commit granularity; no field-level derivation; ad-hoc logs | real git repo of the same session | derivation recovery; replay fidelity; integrity | **Done:** ANFS recovers 100% of the final artifact's derivation closure (4/4 sources) and replays any event boundary; git's structure recovers 0% of derivation edges (file history only), integrity clean. *Larger: field-level derivation + answer-citation chains.* |
