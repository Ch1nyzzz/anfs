...
| --- | --- | --- | --- | --- | --- |
| C1 | Field-level policy blocks sensitive data at the kernel, prompt-independent | prompt-only "don't reveal PII"; regex/PII scrubber | no-label corpus (plain FS / vanilla RAG) | content leak rate across retrieval surfaces | **Done (mechanism):** control 100% / ANFS 0%. *Needed: prompt-LLM + scrubber baselines, real PrivacyLens corpus.* |
