...
impl Workspace {
    ...
    #[pyo3(signature = (logical_path, content, citation_refs, tool_call_id=None, policy_label_excludes=None, retrieval_event_ids=None))]
    fn answer(
        &self,
        logical_path: &str,
        content: &[u8],
        ...
        tool_call_id: Option<String>,
        policy_label_excludes: Option<Vec<String>>,
        retrieval_event_ids: Option<Vec<String>>,
    ) -> PyResult<String> {
        self.answer_impl(
            logical_path,
            content,
            ...
            tool_call_id,
            policy_label_excludes.unwrap_or_default(),
            retrieval_event_ids.unwrap_or_default(),
        )
        .map_err(PyErr::from)
    }
}
...
impl Workspace {
    ...
    fn answer_impl(
        ...
        tool_call_id: Option<String>,
        policy_label_excludes: Vec<String>,
        retrieval_event_ids: Vec<String>,
    ) -> AnfsResult<String> {
        let visibility = VisibilityPolicy::new(&policy_label_excludes)?;
        if citation_refs.is_empty() {
            return Err(AnfsError::PolicyDenied(
                "answer citations must not be empty".to_string(),
            ));
        }
        ...
        for citation_ref in citation_refs {
            if citation_ref.trim().is_empty() {
                return Err(AnfsError::PolicyDenied(
                    "answer citation refs must not be empty".to_string(),
                ));
            }
            if !seen.insert(citation_ref.clone()) {
                return Err(AnfsError::PolicyDenied(format!(
                    "duplicate answer citation ref {citation_ref}"
                )));
            }
            let rec = require_ref(&tx, &citation_ref)?;
            require_path_readable(self, &citation_ref, &rec)?;
            ...
            citations.push((citation_ref, rec));
        }

        let blob_storage = materialize_blob(&self.inner.objects_dir, &hash, content)?;
        validate_answer_retrieval_events(&tx, self, &citations, &retrieval_event_ids)?;
        insert_blob(&tx, &hash, content.len() as i64, &blob_storage)?;
        let node_id = new_node_id();
        let citation_payload: Vec<_> = citations
            .iter()
            .map(|(citation_ref, rec)| {
                json!({
                    "ref_name": citation_ref,
                    "node_id": rec.node_id,
                    "state": rec.state,
                    "ref_version": rec.ref_version,
                })
            ...
            .collect();
        let mut citation_tokens = 0_i64;
        for (_citation_ref, rec) in &citations {
            let size = tx.query_row(
                "
                SELECT b.size
                FROM nodes n
                JOIN blobs b ON b.hash = n.blob_hash
            ...
            citation_tokens += estimated_token_count(size);
        }
        let answer_tokens = estimated_token_count(content.len() as i64);
        let token_accounting = json!({
            "schema": "anfs.token_accounting.estimate.v1",
            "estimator": "ceil_bytes_div_4",
            "answer_tokens": answer_tokens,
            "citation_tokens": citation_tokens,
            "total_tokens": answer_tokens + citation_tokens,
            "citation_count": citations.len() as i64,
            "retrieval_event_count": retrieval_event_ids.len() as i64,
        });
        let metadata_json = json!({
            "schema": "anfs.answer_node.v1",
            "citations": citation_payload.clone(),
            "retrieval_event_ids": retrieval_event_ids.clone(),
        ...
        let citation_node_ids: Vec<String> = citations
            .iter()
            .map(|(_citation_ref, rec)| rec.node_id.clone())
            .collect();
        propagate_fragment_policy_labels_for_blob(
            &tx,
            &node_id,
        ...
        for (idx, (citation_ref, rec)) in citations.iter().enumerate() {
            let role = format!("answer_citation:{idx}");
            insert_edge(
                &tx,
                &event_id,
                "input",
...
fn validate_answer_retrieval_events(
    tx: &Transaction<'_>,
    workspace: &Workspace,
    citations: &[(String, RefRecord)],
    retrieval_event_ids: &[String],
) -> AnfsResult<()> {
    if retrieval_event_ids.is_empty() {
        return Ok(());
    }
    let mut seen = HashSet::new();
    for retrieval_event_id in retrieval_event_ids {
        if retrieval_event_id.trim().is_empty() {
            return Err(AnfsError::PolicyDenied(
                "answer retrieval event ids must not be empty".to_string(),
            ));
        }
        if !seen.insert(retrieval_event_id.clone()) {
            return Err(AnfsError::PolicyDenied(format!(
                "duplicate answer retrieval event id {retrieval_event_id}"
            )));
        }
        let event: Option<(String, Option<String>)> = tx
            .query_row(
                "SELECT kind, workspace_id FROM events WHERE event_id = ?1",
                params![retrieval_event_id],
                |row| Ok((row.get(0)?, row.get(1)?)),
            )
            .optional()?;
        let (kind, workspace_id) =
            event.ok_or_else(|| AnfsError::EventNotFound(retrieval_event_id.clone()))?;
        if kind != "query" && kind != "search" {
            return Err(AnfsError::PolicyDenied(format!(
                "answer retrieval event {retrieval_event_id} must be query or search; found {kind}"
            )));
        }
        if workspace_id.as_deref() != Some(workspace.workspace_name.as_str()) {
            return Err(AnfsError::PolicyDenied(format!(
                "answer retrieval event {retrieval_event_id} is not from workspace {}",
                workspace.workspace_name
            )));
        }
    }

    for (citation_ref, rec) in citations {
        let mut covered = 0;
        for retrieval_event_id in retrieval_event_ids {
            covered += tx.query_row(
                "
                SELECT COUNT(*)
                FROM event_edges ee
                WHERE ee.event_id = ?1
                ...
                ",
                params![retrieval_event_id, rec.node_id, citation_ref],
                |row| row.get::<_, i64>(0),
            )?;
        }
        if covered == 0 {
            return Err(AnfsError::PolicyDenied(format!(
                "answer citation {citation_ref} was not covered by retrieval events"
            )));
        }
    }
    Ok(())
